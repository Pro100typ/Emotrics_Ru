...
def find_point_in_lips(points_upper, points_lower, points_upper_inside,
    ...
    # Нахождение пересечения окружности (радиус radius с центром в углу рта)
    # со сплайном верхней губы
    rot_matrix = np.array([[np.cos(rot_angle), np.sin(rot_angle)],
                           [-np.sin(rot_angle), np.cos(rot_angle)]])
    rot_matrix_inv = np.array([[np.cos(rot_angle), -np.sin(rot_angle)],
                               [np.sin(rot_angle), np.cos(rot_angle)]])
    ...
    new_x_upper += displacement[0]
    new_y_upper += displacement[1]
    new_point_upper = np.array([new_x_upper, new_y_upper])
    
    # Нахождение точки на нижней губе
    x = points_lower[:, 0]
    y = points_lower[:, 1]
    ...
    new_x_lower += new_x_upper
    new_y_lower += new_y_upper
    new_point_lower = np.array([new_x_lower, new_y_lower])
    
    # Внутренняя верхняя губа (видимость зубов)
    ...
    new_x_upper_inside += new_x_upper
    new_y_upper_inside += new_y_upper
    new_point_upper_inside = np.array([new_x_upper_inside, new_y_upper_inside])
    
    # Внутренняя нижняя губа
    ...
    new_x_lower_inside += new_x_upper
    new_y_lower_inside += new_y_upper
...
def palpebral_fissure_height(eye, rot_angle, center):
    ...
    lower_lid_x += center[0]
    lower_lid_y += center[1]
...
def get_measurements_from_data(shape, left_pupil, right_pupil, CalibrationType, CalibrationValue):
    ...
    (new_point_upper_left, new_point_lower_left, new_point_upper_inside_left,
     new_point_lower_inside_left, openness_left, theet_show_left) = find_point_in_lips(
        ...
        rot_angle, shape[54], distance_left)
    ResultsLeft.DentalShow = theet_show_left
    
    # Правая сторона
    (new_point_upper_right, new_point_lower_right, new_point_upper_inside_right,
     new_point_lower_inside_right, openness_right, theet_show_right) = find_point_in_lips(
        np.column_stack((x1_upperlip, y1_upperlip)),
        np.column_stack((x1_lowerlip, y1_lowerlip)),
        np.column_stack((x1_upperlip_inside, y1_upperlip_inside)),
        np.column_stack((x1_lowerlip_inside, y1_lowerlip_inside)),
        rot_angle, shape[48], distance_right)
    ...
    if CalibrationType == 'Iris':
        Calibration = CalibrationValue / (2 * radius)
    else:
        Calibration = CalibrationValue
    
    ResultsLeft.CommissureExcursion *= Calibration
    ResultsLeft.DentalShow *= Calibration
    ResultsLeft.MarginalReflexDistance1 *= Calibration
    ResultsLeft.MarginalReflexDistance2 *= Calibration
    ResultsLeft.BrowHeight *= Calibration
    ResultsLeft.PalpebralFissureHeight = PalpebralFissureHeight_Left * Calibration
    
    ResultsRight.CommissureExcursion *= Calibration
    ResultsRight.DentalShow *= Calibration
    ResultsRight.MarginalReflexDistance1 *= Calibration
    ResultsRight.MarginalReflexDistance2 *= Calibration
    ResultsRight.BrowHeight *= Calibration
    ResultsRight.PalpebralFissureHeight = PalpebralFissureHeight_Right * Calibration
    
    ResultsDeviation.CommisureHeightDeviation *= Calibration
    ResultsDeviation.UpperLipHeightDeviation *= Calibration
    ResultsDeviation.LowerLipHeightDeviation *= Calibration
    ...
    # Процентные отклонения (здоровая сторона – та, где центральная точка губы смещена)
    if shape[57, 0] >= cross_lowerlip[0]:
        # Левая сторона здоровая
        ResultsPercentile.BrowHeight = abs(ResultsLeft.BrowHeight - ResultsRight.BrowHeight) * 100 / ResultsLeft.BrowHeight if ResultsLeft.BrowHeight > 0 else 0
        ResultsPercentile.MarginalReflexDistance1 = abs(ResultsLeft.MarginalReflexDistance1 - ResultsRight.MarginalReflexDistance1) * 100 / ResultsLeft.MarginalReflexDistance1 if ResultsLeft.MarginalReflexDistance1 > 0 else 0
        ResultsPercentile.MarginalReflexDistance2 = abs(ResultsLeft.MarginalReflexDistance2 - ResultsRight.MarginalReflexDistance2) * 100 / ResultsLeft.MarginalReflexDistance2 if ResultsLeft.MarginalReflexDistance2 > 0 else 0
        ResultsPercentile.CommissureExcursion = abs(ResultsLeft.CommissureExcursion - ResultsRight.CommissureExcursion) * 100 / ResultsLeft.CommissureExcursion if ResultsLeft.CommissureExcursion > 0 else 0
        ResultsPercentile.SmileAngle = abs(ResultsLeft.SmileAngle - ResultsRight.SmileAngle) * 100 / ResultsLeft.SmileAngle if ResultsLeft.SmileAngle > 0 else 0
        ...
        # Правая сторона здоровая
        ResultsPercentile.BrowHeight = abs(ResultsLeft.BrowHeight - ResultsRight.BrowHeight) * 100 / ResultsRight.BrowHeight if ResultsRight.BrowHeight > 0 else 0
        ResultsPercentile.MarginalReflexDistance1 = abs(ResultsLeft.MarginalReflexDistance1 - ResultsRight.MarginalReflexDistance1) * 100 / ResultsRight.MarginalReflexDistance1 if ResultsRight.MarginalReflexDistance1 > 0 else 0
        ResultsPercentile.MarginalReflexDistance2 = abs(ResultsLeft.MarginalReflexDistance2 - ResultsRight.MarginalReflexDistance2) * 100 / ResultsRight.MarginalReflexDistance2 if ResultsRight.MarginalReflexDistance2 > 0 else 0
        ResultsPercentile.CommissureExcursion = abs(ResultsLeft.CommissureExcursion - ResultsRight.CommissureExcursion) * 100 / ResultsRight.CommissureExcursion if ResultsRight.CommissureExcursion > 0 else 0
        ResultsPercentile.SmileAngle = abs(ResultsLeft.SmileAngle - ResultsRight.SmileAngle) * 100 / ResultsRight.SmileAngle if ResultsRight.SmileAngle > 0 else 0
